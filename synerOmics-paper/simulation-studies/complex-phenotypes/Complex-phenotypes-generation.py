...
BASEDIR = "/Syneromics" 
CSV = BASEDIR + "/data/ProCan-protein_matrix_6692_averaged.csv"
SAMPLE_COLUMN = 'Cell_Line'
PROTEIN_ID_COLUMN = 'protein_id'
PHENOTYPE_COLUMN = 'ln_IC50'

MAX_ORDER = 2 # TODO:order always 2 for this experiment, CAN BE CHANGED 
MAX_TERMS = 4 #protein effects between these sets are assumed to be independent, as in no cross talks and no rate limiting steps
...
def generate_complex_phenotype(protein_data: pd.DataFrame, 
    ...
    for _ in range(num_protein_sets):
        order = 2 #rng.integers(2, max_order + 1)
...
def main() -> None:
    ...
    OUTPUT_DIR = BASEDIR + "/scripts/results/simulated-phenotypes-her-" + str(PERCENT_EXPLAINABILITY) + "_" + VERSION
    ...
    print("Generating complex phenotypes and saving to CSV files...")
    
    # Generate and write phenotypes for each interaction model
    ...
    # Generate and write complex phenotypes
