...
write_output <- function(data, filename) {
  # Replace dots with semicolons in column names
...
get_parent_child_pairs <- function(tree_info) {
  ...
  # Replace dots with semicolons in splitvarName
  tree_info[, splitvarName := gsub("\\.", ";", splitvarName)]
  
  # Parent-child pairs using the original script's approach
...
for (i in 1:num_trees) {
  ...
  # Get tree information for the current tree
  ...
  # Only occasionally run garbage collection to free memory (not every iteration)
  if (i %% 200 == 0) {
    gc()
  }
}
...
# Also save the detailed parent-child pair information
...
# Additional analysis: Variable usage by tree level
